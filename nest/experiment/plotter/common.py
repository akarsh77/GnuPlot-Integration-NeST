...
def generate_and_execute_gnuplot_script(pltline, path_plt, path_eps):
    directory = os.getcwd() + "/"
    with open(path_plt, "w") as pltfile:
        pltfile.write(pltline)

    run_gnuplot(path_plt)
...
def simple_gnu_plot(path_dat, path_plt, path_eps, x_label, y_label, legend, title=""):
    ...
    pltline = (
        f"set term postscript eps  color blacktext 'Arial'\n"
        f'set output "{os.getcwd()}/{path_eps}"\n'
        f"set title '{title}'\n"
        f'set xlabel "{x_label}"\n'
        f'set ylabel "{y_label}"\n'
        f"set key right top vertical\n"
    ...
    generate_and_execute_gnuplot_script(pltline, path_plt, path_eps)
...
def mix_gnu_plot(dat_list, path_plt, path_eps, x_label, y_label, legend_list, title=""):
    ...
    # Use list comprehension for aggregate x values
    x = np.unique(np.concatenate([chunk["values"][0] for chunk in dat_list]))
    ...
    for datfile, legend_string in zip(dat_list, legend_list):
        ...
        color = "rgb(" + str(a) + "," + str(b) + "," + str(c) + ")"
        splot = (
            f'"{os.getcwd()}/{datfile}" title "{legend_string}" with lines lw 0.5 lc {color},'
        )
        pltline += splot

    pltline = pltline[:-1]  # Remove the trailing comma
